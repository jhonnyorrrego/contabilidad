...
include_once($atras."ventanas/permiso/librerias_reporte_permiso.php");
...
if(@$_REQUEST["observaciones_filtro"]){
  $where_contenedor[] = " and a.observaciones like '%" . $_REQUEST["observaciones_filtro"] . "%'";
}
...
$sql = "select idper,nombre,etiqueta,observaciones,estado from permiso a where 1=1 " . implode("",$where_contenedor) . " " . $order;
$datos = $conexion -> listar_datos($sql,$inicio,$cantidad);

$arreglo = array();

//Obteniendo el total de registros de la consulta
$sql_cantidad = "select count(*) as cantidad from permiso a where 1=1 " . implode("",$where_contenedor);
...
for($i=0;$i<$datos["cant_resultados"];$i++){
  $datos[$i]["acciones_permiso"]=(acciones_permiso($datos[$i]["idper"]));
