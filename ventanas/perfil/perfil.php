...
$().ready(function() {
  ...
  $("#guardar_perfil_formulario").click(function(){
    var formulario = $("#perfil_add");
    var resultado = formulario.valid();
    
    if(resultado){
      var data = $(formulario).serializeArray(); // convert form to array
      data.push({name: "ejecutar", value: 'guardar_perfil_formulario'});
...
<div id="capa_perfil_add" class="row">
    ...
    <div class="card">
      <div class="card-body">
        ...
        <form class="" name="perfil_add" id="perfil_add">
          ...
          <button type="button" id="guardar_perfil_formulario" class="btn btn-primary mb-2">Guardar</button>
        </form>
      </div>
    </div>
  </div>
...
<div id="capa_perfil_edit" class="row" style="display:none">
</div>
...
<div class="row">
  <div class="col-md-12 stretch-card">
    <div class="card">
      <div class="card-body">
        <p class="card-title">Lista perfiles</p>
...
$(document).ready(function(){//Se inicializa la tabla con estilos, el alto del documento y se ejecuta la accion para listar datos sobre la tabla
  $(".limpiar_filtro").click(function(){    
    $("#filtro_perfil").find('input:radio, input:checkbox').removeAttr('checked').removeAttr('selected');
    
    jQuery('#filtro_perfil').each(function(){
      this.reset();
    });
...
function procesamiento_listar(){
  ...
  var data = $('#filtro_perfil').serializeObject();
  
  $('#table').bootstrapTable('refreshOptions', {
    url: 'obtener_perfiles.php',
...
<?php echo(pie()); ?>
<?php include_once($atras . "ventanas/perfil/librerias_reporte_perfil_js.php"); ?>